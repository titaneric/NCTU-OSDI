...
void sched_yield(void) {
    ...
    int index = thiscpu->cpu_task ? thiscpu->cpu_rq.cur_task: 0;
    int i;
    for ( i = 0 ; i < thiscpu->cpu_rq.ntask; ++i ) {
        index = (index + 1) % thiscpu->cpu_rq.ntask;

        int pid = thiscpu->cpu_rq.task_queue[index];
